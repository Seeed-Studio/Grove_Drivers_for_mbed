...
#include "grove_digital_light_tsl2561.h"
...
bool GroveDigitalLight::write_setup(void)
{
	...
	cmdbuf[1] = 0x03;
	suli_i2c_write(this->i2c, TSL2561_Address, cmdbuf, 2);  // POWER UP
	cmdbuf[0] = TSL2561_Timing;
	cmdbuf[1] = 0x00;
	suli_i2c_write(this->i2c, TSL2561_Address, cmdbuf, 2);  //No High Gain (1x), integration time of 13ms
	cmdbuf[0] = TSL2561_Interrupt;
	cmdbuf[1] = 0x00;
	suli_i2c_write(this->i2c, TSL2561_Address, cmdbuf, 2);  
	cmdbuf[0] = TSL2561_Control;
	cmdbuf[1] = 0x00;
	suli_i2c_write(this->i2c, TSL2561_Address, cmdbuf, 2);  // POWER Down
...
bool GroveDigitalLight::read_lux(uint32_t *lux)
{
	...
	suli_i2c_write(this->i2c, TSL2561_Address, cmdbuf, 2);  // POWER UP
	suli_delay_ms(14);
	grove_digital_light_getlux(this->i2c);
...
void GroveDigitalLight::grove_digital_light_getlux(I2C_T *i2c)
{
...
unsigned long GroveDigitalLight::grove_digital_light_calculatelux(unsigned int iGain, unsigned int tInt, int iType)
{
	switch (tInt)
	{
		case 0:  // 13.7 msec
